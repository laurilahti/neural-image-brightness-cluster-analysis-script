...
#   This current version (20250320a) of the script with adaptive thresholding is completed and published by Lauri Lahti at https://github.com/laurilahti/neural-image-brightness-cluster-analysis-script/blob/main/neural-image-brightness-cluster-analysis-script-with-adaptive-thresholding-developed-by-lauri-lahti.R on 20 March 2025. A previous version (20230111c) of the script with global thresholding is completed and published by Lauri Lahti at https://github.com/laurilahti/neural-image-brightness-cluster-analysis-script/blob/main/neural-image-brightness-cluster-analysis-script-developed-by-lauri-lahti.R on 11 January 2023.

#   Please kindly note: This current version (20250320a) of the script is intended primarily for testing purposes and a later version of the script is expected to have further functionality. 

#   If the running of the script "neural-image-brightness-cluster-analysis-script-with-adaptive-thresholding-developed-by-lauri-lahti.R" produces error messages this may be related to having some conflicting settings. Some error messages can originate from diverse file type codings used in the image files which can be difficult to interpret by the script. The errors concerning these image file type codings may become solved by running the script "supplementary-script-for-reading-and-writing-files-of-inputimages-and-maskimages-developed-by-lauri-lahti.R" before running the script "neural-image-brightness-cluster-analysis-script-with-adaptive-thresholding-developed-by-lauri-lahti.R".
...
analysis_list_of_brightness_thresholdvalue_relying_on_the_moving_rectangular_window_and_thresholding_offset_from_the_averaged_value <- c( 0.10 )

half_width_of_moving_rectangular_window_for_adaptive_thresholding <- 10
...
for( counter_index_for_analysis in 1:length(analysis_list_of_filenames_initial) ) {
   ...
   for( counter_index_for_max_number_of_pixels_in_segment_so_that_this_segment_still_needs_to_be_removed in 1:length( analysis_list_of_max_number_of_pixels_in_segment_so_that_this_segment_still_needs_to_be_removed ) ) {
      ...
      for( counter_index_for_brightness_thresholdvalue in 1:length(analysis_list_of_brightness_thresholdvalue_relying_on_the_moving_rectangular_window_and_thresholding_offset_from_the_averaged_value) ) {
...
brightness_thresholdvalue <- analysis_list_of_brightness_thresholdvalue_relying_on_the_moving_rectangular_window_and_thresholding_offset_from_the_averaged_value[ counter_index_for_brightness_thresholdvalue ]
...
c2_gray_eval_brightnessabovethresholdvalue = thresh( c2_gray_masked_pixels, half_width_of_moving_rectangular_window_for_adaptive_thresholding, half_height_of_moving_rectangular_window_for_adaptive_thresholding, brightness_thresholdvalue )
...
print( "The running of the script https://github.com/laurilahti/neural-image-brightness-cluster-analysis-script/blob/main/neural-image-brightness-cluster-analysis-script-with-adaptive-thresholding-developed-by-lauri-lahti.R has ended and aimed to generate analysis results." )
